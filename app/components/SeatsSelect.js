export const SeatsSelect = () => {
  ...
  return (
    <div>
      <div className="flex p-2 mb-4 bg-dark-100 justify-center rounded-full font-semibold text-text-400">
        Layar
      </div>
      <div className="grid grid-cols-6 gap-2 mb-6">
        {[...Array(totalSeats)].map((_, i) => {
          ...
          return (
            <span key={`${row}${seatNumber}`} className="bg-dark-100  flex justify-center py-2 text-sm font-semibold  rounded-full">
              {row}{seatNumber}
            </span>
          );
        })}
